...
public class UnblockUserHandler implements BaseHandler {
    ...
    public HttpResponseBuilder handleRequest(ParsedRequest request) {
        ...
        if (userDao.query(new Document("userName", blockDto.getBlocked())).size() == 0) {
            var res = new RestApiAppResponse<>(false, null, "Unblocking unknown user");
            return new HttpResponseBuilder().setStatus("200 OK").setBody(res);
        }

        blockDto.setBlocker(authResult.userName);
        blockDao.unblockUser(blockDto.getBlocker(), blockDto.getBlocked());
